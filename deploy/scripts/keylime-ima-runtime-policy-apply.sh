...
PY
  ...
  python3 - "$tmp" "$host" "$ip" "$uuid" "$selected_policy_id" "$policy_name" "$runtime_policy_name" "$runtime_policy_path" "$update_rc" "$reactivate_rc" <<'PY'
import json
import sys
p, host, ip, uuid, policy_id, policy_name, runtime_policy_name, runtime_policy_path, update_rc, reactivate_rc = sys.argv[1:]
d = json.load(open(p))
item = {
    "host": host,
    "ip": ip,
