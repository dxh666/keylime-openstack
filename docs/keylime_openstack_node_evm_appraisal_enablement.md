...
restrict `.ima` and `.evm` to certificates that chain to a kernel builtin,
secondary, platform, or machine owner key. If `load-keys` reports:

```text
add_key: Required key not available
```

the kernel rejected the certificate because it is not yet trusted by the
machine trust chain.
