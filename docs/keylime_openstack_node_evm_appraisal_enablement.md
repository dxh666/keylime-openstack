...
Stage 2: sign selected OpenStack/Kolla host runtime files
Stage 3: boot audit-only appraisal with ima_appraise=log
Stage 4: run the host integrity probe and sync the OpenStack trust plane
Stage 5: optional enforcement with ima_appraise=enforce after repeated green runs
```
...
for entry in csri8=172.31.100.8 csri9=172.31.100.9 hygon22=172.31.100.22; do
  ...
  ssh root@"$ip" /usr/local/sbin/keylime-node-evm-appraisal install-key-loader
  ssh root@"$ip" systemctl start keylime-node-evm-keyload.service
  ssh root@"$ip" /usr/local/sbin/keylime-node-evm-appraisal status
done
```
...
```text
keyctl list %:.ima  shows at least one asymmetric key
keyctl list %:.evm  shows at least one asymmetric key
```

If a node refuses `.evm` loading, stop there and inspect kernel config and
dmesg before touching appraisal enforcement.
